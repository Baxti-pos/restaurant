...
const parsePaymentMethod = (value, required = false) => {
    if (value === undefined) {
        if (required) {
...
const getOrderByIdTx = async (tx, branchId, orderId) => {
...
const getOpenOrderMinimalTx = async (tx, branchId, orderId) => {
    const order = await tx.order.findFirst({
        where: {
            id: orderId,
            branchId
...
const recalcOrderTotalsTx = async (tx, orderId) => {
...
const buildLineTotal = (unitPrice, quantity) => {
...
export const ordersService = {
    ...
    async getById(branchId, orderIdRaw) {
        await ensureActiveBranch(branchId);
    ...
    async closeOrder(params) {
        ...
        return prisma.$transaction(async (tx) => {
            ...
            const finalDiscount = discountAmount === undefined || discountAmount === null
                ? currentDiscount
                : discountAmount;
            const finalTotal = clampMinZero(subtotal.minus(finalDiscount));
            const finalPaid = paidAmount === undefined || paidAmount === null ? finalTotal : paidAmount;
            await tx.order.update({
                where: { id: openOrder.id },
                data: {
                    status: "CLOSED",
