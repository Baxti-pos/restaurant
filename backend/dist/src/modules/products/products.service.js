...
const parseIntField = (value, label) => {
    if (value === undefined) {
        return undefined;
    }
...
const ensureCategoryInBranch = async (branchId, categoryId) => {
...
export const productsService = {
    ...
    async getById(ownerId, branchId, productIdRaw) {
        await ensureOwnedActiveBranch(ownerId, branchId);
    ...
    async update(ownerId, branchId, productIdRaw, payload) {
        await ensureOwnedActiveBranch(ownerId, branchId);
        if (!isObject(payload)) {
            throw new ProductsError(400, "So'rov ma'lumoti yaroqsiz");
        }
    ...
    async remove(ownerId, branchId, productIdRaw) {
        await ensureOwnedActiveBranch(ownerId, branchId);
        const productId = parseRequiredString(productIdRaw, "Mahsulot ID");
        const existing = await prisma.product.findFirst({
            where: {
