...
const getWaiterContext = (req: Request) => {
...
export const meController = {
  async tables(req: Request, res: Response) {
    try {
  ...
  async shiftStart(req: Request, res: Response) {
    try {
      const ctx = getWaiterContext(req);
      if (!ctx) {
        return res.status(401).json({ message: "Autorizatsiya talab qilinadi" });
  ...
  async shiftEnd(req: Request, res: Response) {
    try {
      const ctx = getWaiterContext(req);
      if (!ctx) {
        return res.status(401).json({ message: "Autorizatsiya talab qilinadi" });
  ...
  async addItem(req: Request, res: Response) {
    try {
      ...
      const result = await meService.addItem(
        ctx.waiterId,
        ctx.branchId,
        req.params.orderId,
        req.body
