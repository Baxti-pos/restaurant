...
export const reportsService = {
  ...
  async dashboard(ownerId: string, branchId: string, query?: Record<string, unknown>) {
    ...
    const [tables, activeProductsCount, activeCategoriesCount, activeWaitersCount, openShiftsCount, openOrdersCount, closedOrders, expenses] =
      await Promise.all([
    ...
    const sales = closedOrders.reduce(
      (acc, order) => {
  ...
  async waiterActivity(ownerId: string, branchId: string, query?: Record<string, unknown>) {
    const branch = await ensureOwnedActiveBranch(ownerId, branchId);
    const range = parseDateRange(query, "month");

    const waiterId =
      typeof query?.waiterId === "string" && query.waiterId.trim()
        ? query.waiterId.trim()
        : undefined;
    ...
    const waiterMap = new Map(
      waiters.map((waiter) => [
        waiter.id,
        {
          ...
          lastShiftAt: null as string | null
        }
      ])
    );
